...
<div id="content"><!-- end of content left -->
  ...
  <div id="content_right"  style="margin-left:200px; float:none;">
        ...
        <div class="content_right_280_section margin_right_60">
            <div class="content_header_01"><i>Comments</i></div>
...
$newid=$_GET['nid'];
$rep_query="SELECT * FROM news WHERE id='$newid'";
$rep_validate=mysql_query($rep_query);
			
			$rep_fetch=mysql_fetch_assoc($rep_validate);
			$rep_subject=$rep_fetch['subject'];
			$rep_report=$rep_fetch['report'];
			$rep_time=$rep_fetch['time'];
			$_SESSION['head']=$rep_subject;
			
			echo "<div class='news' style='font-family:Browallia New; font-size:24px; padding:4px; color:navy;'><br>
			
			<p class='boldtxt' style='color:cadetblue;'>$rep_subject</p><br>
			<p class='finetxt'>$rep_report</p>
			  </p>
			<br></div>";
...
echo "<p style='font-style:bold; color:blue; font-size:24px; margin:20px;'>Comments</p>";
while($cmt=mysql_fetch_assoc($sql_run)){
	
	if($mnr>=1){
	$real_cmt=$cmt['comments'];
	$real_cmt_time=$cmt['time'];
//"SELECT * FROM `test` WHERE country IN (\'ghana\')";
	echo "<div class='news' style='font-family:Browallia New; font-size:19px; padding:10px; color:Black; background:aliceblue'>
	<p>
	<br>$real_cmt</br>
	$real_cmt_time</p>
	</div>";
...
$runsql=mysql_query($msql);			
        ...
        <br><br><a href="myforum.php" id="blink">Back to Message</a>    
